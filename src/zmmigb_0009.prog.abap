...
*&---------------------------------------------------------------------*
*& 17/10/2016  Allan T.  Fix shortdump due to Excel FM       P30K908489
*& 19/10/2016  Allan T.  Adjust program for services         P30K908587
*&                       and upload mass line items
*& 26/10/2016  Allan T.  Handle Vendor without language      P30K908611
*& 01/11/2016  Allan T.  Fix Plant memory bug issues         P30K908635
*&---------------------------------------------------------------------*
REPORT  zmmigb_0009 LINE-SIZE 500.
...
*DATA: BEGIN OF gt_fieldcat OCCURS 0.
*        INCLUDE STRUCTURE lvc_s_fcat.
*DATA: END OF gt_fieldcat.
...
*    ' ' 'RM06E-EEIND' lv_eeind,
    ' ' 'RM06E-LPEIN' 'T'.
* --> Start added by Allan T. 01/11/2016 - P30K908635
* Consider Plant being populated by memory!!!
  PERFORM populate_bdc_tab USING:
    ' ' 'RM06E-WERKS' p_werks.
* <-- End added by Allan T. 01/11/2016 - P30K908635
...
* --> Start added by Allan Taufiq 26/10/2016 - P30K908611
    ...
    IF lv_spras IS INITIAL.
      PERFORM populate_bdc_tab USING:
        ...
        ' ' 'BUTTON_INIT' 'X'.
    ENDIF.
* <-- End added by Allan Taufiq 26/10/2016 - P30K908611
  ELSE.
    PERFORM populate_bdc_tab USING:
      'X' 'SAPMM06E'   '0301',
      ' ' 'BDC_CURSOR' 'EKKO-KDATE',
...
* items
  ...
  CONCATENATE 'EKPO-MATKL('  lv_tabixc ')' INTO lv_matkl.
...
*End of replace brianrabe P30K910011
    ...
    PERFORM populate_bdc_tab USING:
      ...
      ' ' lv_matkl     fc_rfq-matkl.   "lv_mara_matkl.
...
* --> Start changed by Allan Taufiq 20/10/2016 -P30K908587
* Change routine into dynamic mass line items creation
*    PERFORM insert_to_bdc_items USING sy-tabix
*                                CHANGING lw_rfq.

    PERFORM insert_bdc_items USING sy-tabix
                             CHANGING lw_rfq.
* <-- End changed by Allan Taufiq 20/10/2016 -P30K908587
...
FORM validate_data  TABLES ft_rfq STRUCTURE ty_rfq
  ...
  LOOP AT ft_rfq INTO lw_rfq.
    ...
    CLEAR: lw_log-icon,  lw_log-txz01, lw_log-eeind, lw_log-matkl,
...
*End of replace brianrabe P30K910011
      ...
      IF sy-subrc NE 0.
        ...
        CLEAR fc_valid.
      ELSE.
        lw_log-matkl = lw_mara-matkl.
      ENDIF.
* Start Added  P30K913857 By HH 17 Aug 2021
      ...
      IF lv_meins NE lw_log-meins.
        ...
        ENDIF.
      ENDIF.
* End Added  P30K913857 By HH 17 Aug 2021
