...
FORM rearrange_data .
*Local data declaration.
  DATA : lt_ekpo  TYPE STANDARD TABLE OF zzmmsg_ekpodet,
         ls_ekpo  TYPE zzmmsg_ekpodet,
...
*        READ TABLE it_asc WITH KEY  matnr = <ls_ekpo>-matnr
*                          ASSIGNING <ls_asc>.
*        IF sy-subrc = 0.
*        ENDIF.
...
*PO processing state
    CASE ls_ekko-procstat.
      WHEN lc_01.
        ...
        ls_final-prodes   = text-003.
      WHEN lc_02.
        ls_final-procstat = ls_ekko-procstat.
        ls_final-prodes   = text-004.
      WHEN lc_03.
        ls_final-procstat = ls_ekko-procstat.
        ls_final-prodes   = text-005.
      WHEN lc_04.
        ls_final-procstat = ls_ekko-procstat.
        ls_final-prodes   = text-006.
      WHEN lc_05.
        ls_final-procstat = ls_ekko-procstat.
        ls_final-prodes   = text-007.
      WHEN lc_08.
        ls_final-procstat = ls_ekko-procstat.
        ls_final-prodes   = text-008.
    ENDCASE.
    APPEND ls_final TO it_final.
    CLEAR ls_final.
  ENDLOOP.

*In case Release code is entered on the selection
*screen, filter out final entries
* Filter by Release codes
  IF s_frgco IS NOT INITIAL.
    LOOP AT it_final INTO ls_final.
      READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r1.
      IF sy-subrc NE 0.
        READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r2.
        IF sy-subrc NE 0.
          READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r3.
          IF sy-subrc NE 0.
            READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r4.
            IF sy-subrc NE 0.
              READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r5.
              IF sy-subrc NE 0.
                READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r6.
                IF sy-subrc NE 0.
                  READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r7.
                  IF sy-subrc NE 0.
                    READ TABLE it_relco INTO ls_relco WITH KEY frgco = ls_final-r8.
                    IF sy-subrc NE 0.
                      DELETE it_final WHERE ebeln = ls_final-ebeln.
                    ENDIF.
                  ENDIF.
                ENDIF.
              ENDIF.
...
*Net Amount
  ...
  ls_fldcat-tabname   = 'IT_FINAL'.
*  ls_fldcat-ref_fieldname = 'NETWR'.
*  ls_fldcat-ref_tabname   = 'EKPO'.
...
* Set order of fields for PO Items
  ...
  LOOP AT it_fldcat_det ASSIGNING <ls_fldcat_det>.
    ...
    CASE <ls_fldcat_det>-fieldname.
      ...
      WHEN text-f42. "Account Assignment Category (KNTTP)

        <ls_fldcat_det>-col_pos = 13.

      WHEN text-f43. "Distribution Indicator for Multiple Account Assignment (VRTKZ)

        <ls_fldcat_det>-col_pos = 14.

      WHEN text-f40. "Icon - Account Assignemnts (ZICON_ACC)

        <ls_fldcat_det>-col_pos = 15.

      WHEN text-f44. "Cost Center (KOSTL)

        <ls_fldcat_det>-col_pos = 16.

      WHEN text-f45. "Cost Center Description (LTEXT)

        <ls_fldcat_det>-col_pos = 17.

      WHEN text-f46. "G/L Account (SAKTO)

        <ls_fldcat_det>-col_pos = 18.

      WHEN text-f47. "G/L Account Description (TXT50)
      ...
      WHEN text-f41. "Company Code (BUKRS)
...
* Loop at Account Assginment FieldCategory
  LOOP AT it_fldcat_acc ASSIGNING <ls_fldcat_acc>.
    ...
    CASE <ls_fldcat_acc>-fieldname.
      ...
      WHEN text-f38. "Sequence Number (ZEKKN)

        <ls_fldcat_acc>-key = c_x. "Key Field

      WHEN text-f39. "Controlling Area (KOKRS)
...
*Call function module to display ALV grid
  ...
  CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
    EXPORTING
      ...
      i_callback_pf_status_set = 'SET_PF_STATUS'            "P30K903020
...
*Select Release codes corresponding sel screen input.
  IF s_frgco IS NOT INITIAL.
    SELECT frggr
           frgco
           frgct
      FROM t16fd
      INTO TABLE it_relc
      WHERE frgco IN s_frgco.                         "#EC CI_SGLSELECT

    IF sy-subrc EQ 0.
      SORT it_relc BY frgco.
      DELETE ADJACENT DUPLICATES FROM it_relc COMPARING frgco.
    ENDIF.
    LOOP AT it_relc INTO ls_relc.
      ls_relco-frgco = ls_relc-frgco.
      APPEND ls_relco TO it_relco.
      CLEAR ls_relco.
    ENDLOOP.
    SORT it_relco.
    DELETE ADJACENT DUPLICATES FROM it_relco.
  ENDIF.
...
*Select all values from T16FD
  ...
  IF sy-subrc EQ 0.
    SORT it_frgct BY frgco frgct.
    DELETE ADJACENT DUPLICATES FROM it_frgct
    COMPARING frgco frgct.
  ENDIF.
ENDFORM.                    " FETCH_RELCOD
...
*   Display PO Item Screen
    CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY' "#EC CI_FLDEXT_OK[2215424] P30K910036
      EXPORTING
        i_callback_program      = sy-cprog
...
FORM fetch_asc .
  ...
  FIELD-SYMBOLS: <ls_asc>   TYPE zzmmsg_ascdet,
                 <ls_ekko>  TYPE ty_ekko,
                 <ls_ekpo>  TYPE zzmmsg_ekpodet,
...
*
*          READ TABLE it_t024w WITH KEY  werks = <ls_asc>-werks
*                                        ekorg = <ls_asc>-ekorg
*                              ASSIGNING <ls_t024w>.
*
*          IF sy-subrc <> 0.
...
* Account Assignment Records
  gs_variant_asi-report = sy-cprog.
  gs_variant_asi-handle = text-ha6.
...
*                      1st Screen
              PERFORM: bdc_dynpro USING 'SAPMM06I' '0100',
                       ...
                       bdc_field  USING 'EINA-LIFNR'
                                        lv_lifnr,
                       bdc_field  USING 'EINA-MATNR' "#EC CI_FLDEXT_OK[2215424] P30K910036
                                        lv_matnr,
...
*              Icon (ACC)
          WHEN text-f40. "Account Assignment (ZICON_ACC)
...
*             Loop at Available Source Contracts
              LOOP AT it_asc ASSIGNING <ls_asc>
...
FORM bdc_field USING fnam fval.
  CLEAR bdcdata.
  bdcdata-fnam = fnam.
  bdcdata-fval = fval. "#EC CI_FLDEXT_OK[2215424] P30K910036
  APPEND bdcdata.
ENDFORM.                    "BDC_FIELD
*End   of P30K903020
